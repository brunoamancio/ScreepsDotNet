...
### In Scope
- ✅ Source Keeper AI (pathfinding, targeting, combat)
- ✅ Invader AI (basic movement and combat patterns)
- ✅ Memory field support (`memory_sourceId`, `memory_move`)
- ✅ Path caching and reuse logic
- ✅ Intent generation from AI step
- ✅ Legacy parity validation

### Out of Scope
- ❌ Advanced invader strategies (complex room claiming, coordinated attacks)
- ❌ Power bank mechanics (already complete - destruction via damage)
- ❌ NPC spawning logic (already complete in E5 Phase 3)
...
- ✅ E5 Phase 3: Keeper lair spawning (keepers spawn but don't move)
- ✅ E6: Engine loop orchestration (AI needs to run in processor loop)
- ✅ E7: Parity validation framework (for testing AI behavior)
- ✅ E8: Observability (for debugging AI decisions)

### Infrastructure Already Available
- ✅ Pathfinder: `IPathfinderService` with native C++ solver (D6)
- ✅ Intent handlers: move, attack, rangedAttack, rangedMassAttack (E2)
- ✅ Room state access: Query objects, filter by type/range (E2)
- ✅ Distance utilities: Room position calculations (E2)
...
**Format (from Node.js):**
```javascript
// Position packing: 6 bits for X, 6 bits for Y → single char
...
**Parity test (1 test):**
- Compare keeper behavior with Node.js fixture (same room state → same intents)
...
4. ✅ 10+ unit tests passing (target selection, movement, attack logic)
5. ✅ Integration test: keeper spawns → moves → attacks
6. ✅ Parity test: keeper behavior matches Node.js fixtures
7. ✅ Keepers defend keeper rooms against player intrusion

### Phase 2 (Invader AI) - If Implemented
1. ✅ `InvaderAiStep` implemented
2. ✅ Invader movement and attack patterns functional
3. ✅ Tests passing (similar to keeper tests)
...
5. **Memory format:** Packed positions (6 bits X, 6 bits Y → char code 32+value)
...
- Spawn → find source → move → attack intruder

### Parity Tests (1 test)
- Compare keeper intents with Node.js engine output
- Same room state → same intent decisions
...
**E7 Plan:** `docs/engine/e7.md` → Parity validation requirements
**Pathfinder Docs:** `src/ScreepsDotNet.Driver/docs/Pathfinder.md`
