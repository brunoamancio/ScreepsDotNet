...
**Status:** ✅ Phase 1 Complete (Keeper AI) | 🟡 Phase 2 Deferred (Invader AI)
...
### Phase 2: Invader AI - 🟡 DEFERRED
- 📋 Basic invader movement patterns (patrol, attack)
- 📋 Target selection (different from keepers)
...
### Parity Tests (E7) - ✅ COMPLETE (2026-01-22)
- **Status:** ✅ 10 parity tests implemented and passing
- **File:** `ScreepsDotNet.Engine.Tests/Parity/Tests/KeeperAiParityTests.cs`
- **Coverage:**
  1. Target assignment near source
  2. Attack lowest HP hostile
  3. Mass attack with multiple hostiles
  4. Ignore invaders (NPC user ID "2")
  5. No movement when no source nearby
  6. Choose nearest source when multiple available
  7. Stop moving when adjacent to source
  8. Combat and movement execute together
  9. Path caching across ticks
  10. Ranged attack from distance
- **Integration:** Added `KeeperAiStep` to `ParityTestRunner` processor pipeline
