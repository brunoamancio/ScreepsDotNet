...
- ✅ D4: Scheduler hooks complete (telemetry integration)
- ✅ E6: Engine orchestration complete
...
### In Scope
- ✅ Engine telemetry payload (room processing metrics)
- ✅ E3 validation statistics export (deferred from E3.4)
- ✅ CLI diagnostics commands (`engine status`, `engine room-state`, `engine validation-stats`)
- ✅ HTTP diagnostics endpoints (`/api/game/engine/*`)
- ✅ Operator playbooks (debugging workflows, error references)
- 🔧 Performance profiling hooks (optional Phase 5)

### Out of Scope
- ❌ Advanced observability (distributed tracing, APM integration) - defer to E8.1
- ❌ Real-time monitoring dashboards - defer to E8.1
- ❌ Log aggregation/search - use existing logging infrastructure
...
- ✅ D8: Runtime telemetry infrastructure (`RuntimeTelemetryPayload`, `IRuntimeTelemetrySink`)
- ✅ D4: Scheduler hooks (`IDriverLoopHooks.PublishRuntimeTelemetryAsync`)
- ✅ E6: Engine orchestration (RoomProcessor pipeline)
- ✅ E3: Validation statistics sink (`IValidationStatisticsSink`, `ValidationStatistics`)

### Already Available Infrastructure
- ✅ D8 telemetry: Stage-tagged metrics, watchdog alerts, observability exporters
- ✅ E3 validation: Statistics collection (in-memory, thread-safe)
- ✅ CLI framework: Spectre.Console, command handlers, formatters
- ✅ HTTP framework: ASP.NET Core minimal APIs, authentication
...
### D8 Telemetry Infrastructure (Complete ✅)
...
### E3 Validation Statistics (Ready to Export ✅)
...
- Bridge ValidationStatistics → RuntimeTelemetryPayload (or new payload type)
- Reset stats per tick or per batch (configurable)

### CLI/HTTP Diagnostic Patterns (Ready to Follow ✅)
...
**Tests (4 endpoints × 2 scenarios = 8 total):**
...
- EngineEndpointsTests: 8 tests (4 endpoints × 2 scenarios)
...
**Tests:**
1. `ProcessRoom_EmitsTelemetry_EndToEnd` - Verify telemetry flows from RoomProcessor → Sink → D8
...
### Phase 1: Engine Telemetry ✅ COMPLETE (2026-01-21)
...
- [x] DI registration complete (`IEngineTelemetrySink` → `EngineTelemetrySink`)
...
### Phase 2: CLI Diagnostics ✅ COMPLETE (2026-01-21)
...
- [ ] `GET /api/game/engine/status` returns Engine statistics
- [ ] `GET /api/game/engine/room-state?room=W1N1` returns room state
- [ ] `GET /api/game/engine/validation-stats` returns validation stats
- [ ] `POST /api/game/engine/validation-stats/reset` clears stats
- [ ] Authentication required for all endpoints
- [ ] 8 unit tests passing
- [ ] Endpoints callable via HTTP client

### Phase 4: Operator Playbooks ✅
- [ ] `docs/engine/operator-playbooks.md` created
- [ ] 5+ debugging playbooks documented
- [ ] Validation error reference complete
- [ ] CLI quick reference included
- [ ] HTTP quick reference with curl examples
- [ ] Cross-referenced in `docs/README.md` and `docs/engine/roadmap.md`
...
### New Files (E8 Phase 1-2: 15 files)
1. `src/ScreepsDotNet.Engine/Telemetry/EngineTelemetryPayload.cs` ✅ Phase 1
2. `src/ScreepsDotNet.Engine/Telemetry/IEngineTelemetrySink.cs` ✅ Phase 1
3. `src/ScreepsDotNet.Engine/Telemetry/EngineTelemetrySink.cs` ✅ Phase 1
4. `src/ScreepsDotNet.Engine/Constants/EngineTelemetryConstants.cs` ✅ Phase 1
5. `src/ScreepsDotNet.Backend.Core/Services/IEngineDiagnosticsService.cs` ✅ Phase 2
6. `src/ScreepsDotNet.Backend.Core/Services/StubEngineDiagnosticsService.cs` ✅ Phase 2 (stub, real impl in E8.1)
7. `src/ScreepsDotNet.Backend.Core/Services/EngineStatisticsSnapshot.cs` ✅ Phase 2
8. `src/ScreepsDotNet.Backend.Cli/Commands/Engine/EngineStatusCommand.cs` ✅ Phase 2
9. `src/ScreepsDotNet.Backend.Cli/Commands/Engine/RoomStateCommand.cs` ✅ Phase 2
10. `src/ScreepsDotNet.Backend.Cli/Commands/Engine/ValidationStatsCommand.cs` ✅ Phase 2
11. `src/ScreepsDotNet.Backend.Http/Endpoints/EngineEndpoints.cs` ⏳ Phase 3
12. `docs/engine/operator-playbooks.md` ⏳ Phase 4
13. `src/ScreepsDotNet.Engine.Tests/Telemetry/EngineTelemetrySinkTests.cs` ✅ Phase 1
14. `src/ScreepsDotNet.Engine.Tests/Processors/RoomProcessorTelemetryTests.cs` ✅ Phase 1
15. `src/ScreepsDotNet.Backend.Cli.Tests/Commands/EngineCommandTests.cs` ✅ Phase 2
16. `src/ScreepsDotNet.Backend.Http.Tests/Endpoints/EngineEndpointsTests.cs` ⏳ Phase 3

### Modified Files (E8 Phase 1-2: 5 files)
1. `src/ScreepsDotNet.Engine/Processors/RoomProcessor.cs` ✅ Phase 1 - Add telemetry emission
2. `src/ScreepsDotNet.Engine/ServiceCollectionExtensions.cs` ✅ Phase 1 - Register telemetry services
3. `src/ScreepsDotNet.Backend.Cli/Application/CliApplication.cs` ✅ Phase 2 - Register engine commands
4. `src/ScreepsDotNet.Backend.Cli/Program.cs` ✅ Phase 2 - Add Engine DI services
5. `src/ScreepsDotNet.Backend.Cli/ScreepsDotNet.Backend.Cli.csproj` ✅ Phase 2 - Add Engine project reference
6. `src/ScreepsDotNet.Backend.Cli.Tests/ScreepsDotNet.Backend.Cli.Tests.csproj` ✅ Phase 2 - Add Engine project reference
7. `src/ScreepsDotNet.Backend.Http/Routing/ApiRoutes.cs` ⏳ Phase 3 - Add engine routes
8. `src/ScreepsDotNet.Backend.Http/Program.cs` ⏳ Phase 3 - Map engine endpoints
9. `docs/README.md` ⏳ Phase 4 - Add operator playbooks entry
10. `docs/engine/roadmap.md` ⏳ Phase 4 - Mark E8 as complete when done
...
2. Update test counts (763 → 783 total tests after Phase 2-3)
...
**Recommendation:** ✅ **Defer to E8.1** - Keep stub for now, implement as enhancement after E8 completion.
...
**Decision:** ✅ Implemented `StubEngineDiagnosticsService` in Phase 2, defer real aggregation to E8.1.
