...
### Phase 1: Engine Telemetry ✅
- [ ] `EngineTelemetryPayload` defined with all required fields
- [ ] `IEngineTelemetrySink` interface and implementation created
- [ ] `RoomProcessor` emits telemetry after each room tick
- [ ] Validation statistics included in telemetry payload
- [ ] Validation stats reset after export
- [ ] 10 unit tests passing
- [ ] Telemetry visible in Driver logs (stage=`engine:room:*`)

### Phase 2: CLI Diagnostics ✅
- [ ] `engine status` command shows overall Engine statistics
- [ ] `engine room-state <roomName>` command shows room snapshot
- [ ] `engine validation-stats` command shows validation metrics
- [ ] `--json` flag works for all commands
- [ ] Commands registered in `RootCommand`
- [ ] 10 unit tests passing
...
### New Files (13 files)
1. `src/ScreepsDotNet.Engine/Telemetry/EngineTelemetryPayload.cs`
2. `src/ScreepsDotNet.Engine/Telemetry/IEngineTelemetrySink.cs`
3. `src/ScreepsDotNet.Engine/Telemetry/EngineTelemetrySink.cs`
4. `src/ScreepsDotNet.Backend.Core/Services/IEngineDiagnosticsService.cs`
5. `src/ScreepsDotNet.Backend.Core/Services/EngineDiagnosticsService.cs`
6. `src/ScreepsDotNet.Backend.Cli/Commands/Engine/EngineStatusCommand.cs`
7. `src/ScreepsDotNet.Backend.Cli/Commands/Engine/RoomStateCommand.cs`
8. `src/ScreepsDotNet.Backend.Cli/Commands/Engine/ValidationStatsCommand.cs`
9. `src/ScreepsDotNet.Backend.Http/Endpoints/EngineEndpoints.cs`
10. `docs/engine/operator-playbooks.md`
11. `src/ScreepsDotNet.Engine.Tests/Telemetry/EngineTelemetrySinkTests.cs`
12. `src/ScreepsDotNet.Engine.Tests/Processors/RoomProcessorTelemetryTests.cs`
13. `src/ScreepsDotNet.Backend.Http.Tests/Endpoints/EngineEndpointsTests.cs`

### Modified Files (7 files)
1. `src/ScreepsDotNet.Engine/Processors/RoomProcessor.cs` - Add telemetry emission
2. `src/ScreepsDotNet.Engine/ServiceCollectionExtensions.cs` - Register telemetry services
3. `src/ScreepsDotNet.Backend.Cli/Commands/RootCommand.cs` - Register engine commands
4. `src/ScreepsDotNet.Backend.Http/Routing/ApiRoutes.cs` - Add engine routes
5. `src/ScreepsDotNet.Backend.Http/Program.cs` - Map engine endpoints
6. `docs/README.md` - Add operator playbooks entry
7. `docs/engine/roadmap.md` - Mark E8 as complete when done
...
2. Update test counts (754 → 794 total tests)
