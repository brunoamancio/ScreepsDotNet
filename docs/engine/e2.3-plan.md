...
## Next Actions
1. Extend driver contracts to expose construction sites (`RoomObjectSnapshot` progress/progressTotal fields and a typed construction-site DTO) so the engine can read pending builds.
2. Port the creep repair/build logic using the new blueprint metadata, including tests.
2. Follow up with the harvest handler and expanded telemetry coverage.
