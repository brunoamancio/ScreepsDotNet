...
Status: updated January 18, 2026. This file tracks the granular work required to finish **E2.3 – "Wire engine consumption"** and its dependencies. It supplements `docs/engine/data-model.md` (data contracts) and `src/ScreepsDotNet.Engine/AGENT.md` (high-level milestones).
...
3. **Global intent handlers (In Progress):** after transfers, port the legacy market/power/global-intent hooks, ensuring stats/history updates stay centralized.
   ...
   - Helper scaffolding (Jan 17 2026): Added `RoomCoordinateHelper`, `TerminalMath`, and `StoreMath` under `ScreepsDotNet.Common.Utilities` (with unit tests) so the upcoming market processor can reuse the legacy `roomNameToXY`, `calcRoomsDistance`, `calcTerminalEnergyCost`, and `_calcResources` logic without drifting.
   - Market intent phase 1 (Jan 17 2026): `MarketIntentStep` now handles the bookkeeping flows (`createOrder`, `cancelOrder`, `changeOrderPrice`, `extendOrder`) with new mutation plumbing (`MarketOrderMutation`, `UserMoneyMutation`, `UserMoneyLogEntry`). Orders, user balances, and fee logs are emitted through `GlobalMutationDispatcher`, matching the Node processor’s fees/activation behavior while keeping all logic inside .NET.
   - Power creep create (Jan 17 2026): `PowerCreepIntentStep` now processes `createPowerCreep` intents, validating name conflicts/class validity and staging power creep mutations through the global writer.
   - Terminal sends (Jan 18 2026): `MarketIntentStep.ProcessTerminalSends` now handles terminal send intents stored on terminal objects, calculating energy costs with exponential distance formula, applying PWR_OPERATE_TERMINAL cooldown reductions, validating resource availability/capacity, and creating transaction log entries for successful transfers.
   - Power constants (Jan 18 2026): Ported `PowerTypes` enum (19 power abilities with C# naming: `GenerateOps`, `OperateSpawn`, etc.) and `PowerInfo.Abilities` dictionary containing comprehensive metadata (level requirements, cooldowns, ranges, ops costs, effect multipliers, durations) to unblock `upgradePowerCreep` and other power-related handlers.
   - Engine-Driver boundary hardening (Jan 18 2026): Removed critical MongoDB layering violation from Engine by creating typed DTOs (`PowerEffectSnapshot`, `TerminalSendSnapshot`) in Driver/Contracts, eliminating all `using MongoDB.Bson;` references from Engine code, and centralizing MongoDB field names as constants (`RoomObject.EffectFields`, `RoomObject.SendFields`).
   - Power creep lifecycle handlers (Jan 19 2026): `PowerCreepIntentStep` now processes all power creep lifecycle intents:
     - **SuicidePowerCreep**: Removes spawned power creeps from rooms, clearing their `Shard` field to mark them as unspawned.
     - **SpawnPowerCreep**: Spawns unspawned power creeps at specified spawn locations, creating room objects and setting their `Shard` field to the spawn's room.
     - **UpgradePowerCreep**: Unlocks new power abilities for power creeps, validating level requirements from `PowerInfo.Abilities` and preventing duplicate unlocks.
   - Market deal intents (Jan 19 2026): `MarketIntentStep` now processes all market order fulfillment via `ProcessDealIntents`, `ProcessTerminalDeals`, and `ProcessDirectDeals`:
     - **Terminal Deals**: Regular resource exchanges (energy, minerals, commodities) via terminals with distance-based sorting, terminal ownership/capacity validation, money transfers, transaction logging, order updates, and PWR_OPERATE_TERMINAL cooldown support.
     - **Direct Deals**: Intershard resource exchanges (SubscriptionToken, CpuUnlock, Pixel, AccessKey) bypassing terminals with user-to-user resource transfers, random shuffle for fairness, money and resource balance updates, and full transaction logging.
     - **UserState.Resources**: Added `Resources` property to `UserDocument` and `UserState` record, updated `RoomContractMapper` to map user resources, enabling direct deal execution.
     - **Infrastructure**: Added `CreateMoneyLogEntry` and `CreateResourceLogEntry` helpers, constants (`MarketSell`, `MarketBuy`, `AnotherUser`), and eliminated all hardcoded strings.
   - **TODO:** Port the remaining legacy handlers (power ability intents, NPC order maintenance) into managed steps and hook their stats/history writes into the new `GlobalMutationWriter` pipeline.
   - **Analysis (Jan 17 2026):** Legacy `global-intents/market.js` handles 5 user intent families (`createOrder`, `cancelOrder`, `changeOrderPrice`, `extendOrder`, `deal`) plus passive terminal sends and NPC order cleanup. It mutates terminals, market orders, user balances/resources, and transaction logs. Parity requires room-name distance math (`roomNameToXY`, wrap-aware Chebyshev distance), the `calcTerminalEnergyCost` exponential, and helpers mirroring `_calcResources` (store totals). These utilities now need managed equivalents before we can port the processor itself.

   **Next steps for the market/global intent port**

   1. ✅ **Contract additions (completed Jan 17-18 2026):** Extended `GlobalMutationBatch`/writer/dispatcher so market steps can emit order mutations, user balance deltas, transaction/history entries, and terminal store updates without reaching into Mongo documents directly.
   2. ✅ **MarketIntentStep phase 1 (completed Jan 17 2026):** Added managed step that iterates `GlobalUserIntentSnapshot` entries and handles the bookkeeping-only flows (`createOrder`, `cancelOrder`, `changeOrderPrice`, `extendOrder`), consuming `RoomCoordinateHelper`/`TerminalMath`/`StoreMath`, validating user funds, staging bulk mutations via dispatcher hooks, and mimicking the Node processor's stat events.
   3. ✅ **Terminal send handling (completed Jan 18 2026):** Expanded `MarketIntentStep` to cover terminal `send` intents (player `deal` intents remain TODO). Maps relevant terminals from context, honors cooldowns/power boosts (`PWR_OPERATE_TERMINAL`), validates resource availability/capacity, and creates transaction log entries.
   4. ✅ **Deal intent handling (completed Jan 19 2026):** Ported player `deal` intents covering both terminal-based deals (regular resources with distance sorting, ownership validation, money transfers, transaction logging) and direct inter-shard deals (intershard resources via user-to-user resource exchanges). Added `UserState.Resources` property, `CreateResourceLogEntry` helper, and all required constants (`MarketSell`, `MarketBuy`, `AnotherUser`).
   5. **NPC order maintenance (TODO):** Port the legacy cleanup path (expiration refunds, activation toggles based on available inventory) so the managed processor keeps order documents consistent without the Node helper.
   6. **Tests & telemetry (TODO):** Add focused unit tests for deal/NPC intents plus dispatcher integration tests to ensure money/resources/stats are written exactly once per path.
