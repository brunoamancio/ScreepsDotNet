...
4. **Movement crash handlers ☐** –
   ...
   - Remaining work: detect true lethal outcomes that depend on cross-room metadata (failed room-edge exits / destination lookup) and any future tile overflow mechanics that require multi-room context, then route those losers through the death processor so stats/logs stay accurate.
     - ✅ Driver exit-map cache now exists: `RoomExitTopologyProvider` scans accessible rooms + static terrain, counts edge exits, records neighbor rooms + accessibility flags, and invalidates when `UpdateAccessibleRoomsListAsync` runs. The map now flows through `InterRoomSnapshot` ➜ `GlobalSnapshot` ➜ `RoomProcessorContext`.
     - ✅ Engine plumbing: `RoomProcessor` fetches the cached `GlobalState` once per tick, attaches the per-room exit descriptor to `RoomProcessorContext`, and `MovementIntentStep` consults it so only exits that lack accessible neighbors trigger a crash.
     - Next: once we support cross-room transfers, reuse the same topology to hand the creep off to the global processor instead of treating accessible exits as blocked.
   - Build a `MovementResolutionStep` that mirrors the legacy matrix merge: gather every `move` intent, clamp to room bounds, and resolve conflicts (pulling priority, fatigue/weight ratios, safe-mode ramparts, etc.).
   - Detect lethal outcomes during resolution:
     1. **Tile overflows** – if multiple creeps end up on an unwalkable tile (wall, closed rampart) or a tile already occupied by an immovable structure, treat the extra occupants as crashes and call the death processor.
     2. **Room-edge failures** – if a creep attempts to exit the room but the target shard/room is missing, drop it via the processor (legacy burn).
   - Emit the same stats/action-log entries the Node `_die` helper would (event log `die`, `creepsLost`, tombstones/drops) so telemetry stays aligned.
   - Add unit tests covering the remaining crash cases (multi-creep collisions, edge failures) to lock the parity down before wiring the step into `AddEngineCore`.
