...
1. Introduce shared structure/boost metadata so the upcoming repair/build handlers can insert finished structures with correct stats (hits, store capacity, decay data).
   - Add a common `StructureBlueprint` record plus registry under `ScreepsDotNet.Common` that captures per-structure defaults (hits/hitsMax, store/storeCapacityResource, decay/cooldown timers, ownership flags, rampart per-RCL tables, etc.).
   - Populate it using the authoritative values from the legacy Node constants (`common/lib/constants.js`) and backend `GameConstants`, with unit tests covering representative structures (spawn, extension, road, rampart, container, lab).
   - Expose an `IStructureBlueprintProvider` via DI so engine steps (and eventually driver mappers) can fetch blueprint data and build `RoomObjectSnapshot`s without duplicating constants.
