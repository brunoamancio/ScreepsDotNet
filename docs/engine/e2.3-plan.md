...
3. **Global intent handlers (In Progress):** after transfers, port the legacy market/power/global-intent hooks, ensuring stats/history updates stay centralized.
   ...
   - **TODO:** Port the remaining legacy handlers (market order fulfillment, terminal/factory power flows, ops/power ability intents) into managed steps and hook their stats/history writes into the new `GlobalMutationWriter` pipeline.
   ...
   **Next steps for the market/global intent port**
   ...
   1. **Contract additions:** extend `GlobalMutationBatch`/writer/dispatcher so market steps can emit order mutations, user balance deltas, transaction/history entries, and terminal store updates without reaching into Mongo documents directly.
   2. **MarketIntentStep phase 1:** add a managed step that iterates `GlobalUserIntentSnapshot` entries and handles the bookkeeping-only flows (`createOrder`, `cancelOrder`, `changeOrderPrice`, `extendOrder`). This step will consume `RoomCoordinateHelper`/`TerminalMath`/`StoreMath`, validate user funds, stage bulk mutations via the new dispatcher hooks, and mimic the Node processor’s stat events.
   3. **Terminal send/deal handling:** once phase 1 lands, expand the step to cover terminal `send` intents and player `deal` intents (both terminal-based and direct inter-shard). This requires mapping the relevant terminals from `SpecialRoomObjects`, honoring cooldowns/power boosts, and staging room-level patches/mutations.
   4. **NPC order maintenance:** port the legacy cleanup path (expiration refunds, activation toggles based on available inventory) so the managed processor keeps order documents consistent without the Node helper.
   5. **Tests & telemetry:** add focused unit tests for each intent family plus dispatcher integration tests to ensure money/resources/stats are written exactly once per path. Update docs/memory after each phase so future sessions know which portions remain (e.g., factory/power intent hooks).
