...
## Next Actions
1. Port the harvest handler (sources, minerals, deposits) so intent energy flows and stats (`energyHarvested`) are recorded through the new helpers.
2. Extend telemetry/history listeners to emit the new spawn/tombstone/energy counters so downstream dashboards stay accurate once harvest stats land.
3. Continue expanding death coverage (movement crashes, lifecycle expirations) so every lethal path routes through `ICreepDeathProcessor`.
