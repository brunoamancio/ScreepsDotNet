...
**Status:** Phase 1 Complete ✅ (2026-01-21) - User Stats Mutations Operational
...
**E4 Reference:** `docs/engine/e4.md` → "Deferred Features"
**Parity Status:** ✅ **PARITY-CRITICAL** (required for E7 validation)
**Legacy Source:** `ScreepsNodeJs/engine/src/processor/intents/keeper-lairs/tick.js`

**Why originally deferred:**
- ~~Invader spawning depends on global room status tracking~~ ❌ **INCORRECT ASSUMPTION**
- ~~Source keeper spawning uses global room timers~~ ❌ **INCORRECT ASSUMPTION**
...
**Why originally deferred:**
- ~~Power bank decay coordinated with source keeper spawning~~ ❌ **INCORRECT ASSUMPTION**
- ~~Affects global power economy~~ ⚠️ **PARTIALLY CORRECT** (but decay is room-local)
...
**What's needed:**
- Power bank destruction already handled by `_damage.js` → `_destroy.js` flow
...
**Deferred from:** E4 (Simulation Kernel - Room Processor)
**E4 Reference:** `docs/engine/e4.md` → "Deferred Features"
**Parity Status:** ⚠️ **LOWER PRIORITY** (advanced structure mechanic)
**Legacy Source:** `ScreepsNodeJs/engine/src/processor/intents/nukers/launch-nuke.js`, `nukes/tick.js`, `nukes/pretick.js`

**Why originally deferred:**
- ~~Requires global nuke tracking~~ ⚠️ **PARTIALLY CORRECT** (but nukes are room objects, not global state)
...
### 1. Global Mutation Infrastructure ⚠️ **95% COMPLETE**
...
public interface IGlobalMutationWriter
{
    ...
    // ❌ MISSING: User GCL/power methods (needed for E5 Phase 1)
...
- ✅ `IGlobalMutationWriter` injected into `RoomProcessorContext` (via Driver's `IGlobalMutationDispatcher`)
- ✅ Flush called after all room processors complete (in main loop global processor)
- ✅ Batching infrastructure mirrors room mutation pattern
...
### Phase 1: User Stats Mutations (Unblocks E2.3) ✅ COMPLETE (2026-01-21)
...
1. ✅ Added 3 methods to `IGlobalMutationWriter` (`IncrementUserGcl`, `IncrementUserPower`, `DecrementUserPower`)
2. ✅ Created Driver contracts (`UserGclMutation`, `UserPowerMutation`) and updated `GlobalMutationBatch`
3. ✅ Implemented `GlobalMutationWriter` methods with validation and batching
4. ✅ Wired into `GlobalMutationDispatcher.ApplyAsync()` using `$inc` MongoDB operator
5. ✅ Added 19 comprehensive unit tests (all passing)
6. ✅ Implemented all 4 E2.3 unblocked features:
   - ✅ Controller GCL updates (`ControllerIntentStep` line 136)
   - ✅ Boost effects GCL component (included in controller upgrades)
   - ✅ PWR_GENERATE_OPS ability (`PowerAbilityStep` lines 505-585) with overflow drops
   - ✅ Power spawn balance tracking (`PowerSpawnIntentStep` line 111)
...
### Phase 2: Power Effect Tracking ✅ COMPLETE (E2.3)
~~**Effort:** 1-2 days~~
**Status:** ✅ **NO LONGER NEEDED** - Completed in E2.3 as part of Power Abilities work

**What was implemented:**
- ✅ `PowerEffectDecayStep` - Removes expired effects each tick
- ✅ `PowerAbilityStep` - Applies power effects to structures
- ✅ Effect consumption in Lab/PowerSpawn/Factory handlers
- ✅ 67 tests covering power effect lifecycle

---

### Phase 3: Keeper Lairs ✅ COMPLETE (2026-01-21)
**Effort:** 2-3 hours estimated → **2.5 hours actual**
**Status:** ✅ Complete - all keeper lair spawn mechanics operational
**Unblocks:** E4 feature #8 (keeper room mechanics)

**Completed Deliverables:**
1. ✅ `KeeperLairStep` processor (1.5 hours):
...
3. ✅ Registration in DI (15 minutes):
...
**Deferred Features:**
- 📋 **Keeper AI logic** (pathfinding, targeting) - deferred to post-MVP
...
- 📋 **Power Bank mechanics** - NOT part of E5 Phase 3
...
### Phase 4: Nuker Launch & Landing 📋 LOWER PRIORITY
...
**Dependencies:**
- E5 Phase 1 (for `IGlobalMutationWriter.UpsertRoomObject()`) - ✅ Already complete!
- Cross-room mutations already supported

---

### Phase 5: Market & Shard Messaging 📋 LOWEST PRIORITY
**Effort:** 3-5 days
...
**Master Roadmap:** `docs/engine/roadmap.md` → "E5: Global Systems"
...
1. ✅ Add 3 methods to `IGlobalMutationWriter`: `IncrementUserGcl`, `IncrementUserPower`, `DecrementUserPower`
2. ✅ Create `UserGclMutation` and `UserPowerMutation` contract records
3. ✅ Update `GlobalMutationBatch` to include new mutation lists
4. ✅ Implement batching logic in `GlobalMutationWriter`
5. ✅ Implement `BulkUserStatsWriter` in storage layer
6. ✅ Wire into `IGlobalMutationDispatcher.ApplyAsync()` flush logic
7. ✅ 22 tests pass (10 GCL + 8 power + 4 batching)
8. ✅ Return to E2.3 and unblock 4 features:
   - Controller GCL updates (1 hour)
...
1. ✅ Power effect decay logic implemented (E2.3)
2. ✅ PWR_OPERATE_LAB boosts lab reaction amounts (E2.3)
3. ✅ PWR_OPERATE_POWER boosts power spawn processing (E2.3)
4. ✅ PWR_OPERATE_FACTORY boosts factory level (E2.3)

### Phase 3 (Medium Priority) - Keeper Lairs:
1. ✅ `KeeperLairStep` processor implemented (2 hours)
2. ✅ Keeper spawn logic mirrors legacy behavior (name, body, timing)
3. ✅ 8 tests pass covering spawn conditions and keeper creation
4. ✅ Step registered in DI with correct ordering
5. ✅ Power bank destruction verified (1 hour testing - no new code needed)

### Phase 4 (Lower Priority) - Nuker Launch & Landing:
1. ✅ `NukerIntentStep` intent handler (E2 scope - 2 hours)
2. ✅ `NukeLandingStep` passive processor (E4 scope - 2 hours)
3. ✅ Nuke objects created in target rooms via `IGlobalMutationWriter.UpsertRoomObject()`
4. ✅ Landing logic applies damage, kills creeps, blocks controller
5. ✅ 8-10 tests pass covering launch, flight, and landing

### Phase 5 (Lowest Priority) - Market & Shard Messaging:
1. 📋 Market NPC order generation (deferred indefinitely for private server MVP)
2. 📋 Shard messaging infrastructure (deferred indefinitely)
