...
**Purpose:** Implement lockstep testing between the .NET Engine and legacy Node.js engine to ensure 100% behavioral parity. This validates that the .NET Engine produces identical simulation results for all game mechanics.
...
**Current State:**
- .NET Engine has 428 passing unit/integration tests
...
│   └── ...
├── tools/
│   └── parity-harness/          # NEW: Self-contained parity testing
│       ├── scripts/             # All parity scripts here
│       │   ├── clone-repos.sh
│       │   └── run-parity-tests.sh
...
**File:** `tools/parity-harness/scripts/clone-repos.sh`
...
**File:** `tools/parity-harness/test-runner/fixture-loader.js`
...
**File:** `tools/parity-harness/test-runner/processor-executor.js`
...
**File:** `tools/parity-harness/test-runner/output-serializer.js`
...
**File:** `tools/parity-harness/test-runner/run-fixture.js`
...
**Goal:** Create comprehensive fixtures covering all game mechanics.

#### Step 4.1: Core Mechanics Fixtures (3 hours)

**Fixtures to Create (30-40 fixtures):**
...
**Controller (5 fixtures):**
- Upgrade (normal, boosted)
- Downgrade mechanics
- Level transitions
- Safe mode
- Reservation

**Resource I/O (3 fixtures):**
...
- Power abilities (sample of 18 abilities)
- Effect decay

**Structures (4 fixtures):**
- Link transfers
- Factory production
- Nuker launch/landing
- Keeper spawning

#### Step 4.2: Edge Cases (2 hours)

**Fixtures to Create (15-20 fixtures):**
- Empty stores
...
**File:** `tools/parity-harness/scripts/clone-repos.sh` (updated)
```bash
#!/bin/bash
set -e
...
### Phase 4 (Coverage)
1. ✅ 40+ parity fixtures covering all mechanics
2. ✅ All E2 handler families have parity tests
3. ✅ E3 validator parity tests created
4. ✅ Edge cases covered (overflow, TTL, conflicts)
...
- [ ] Step 1.2: Fixture Loader
- [ ] Step 1.3: Processor Executor
- [ ] Step 1.4: Output Serializer
- [ ] Step 1.5: CLI Wrapper
...
**Roadmap:** `docs/engine/roadmap.md` → E7 entry
**E2 Plan:** `docs/engine/e2.md` → Handler coverage details
**E3 Plan:** `docs/engine/e3.md` → Validation parity deferred work
**Official Screeps Repos:**
- Engine: https://github.com/screeps/engine
- Driver: https://github.com/screeps/driver
- Common: https://github.com/screeps/common
**Test Infrastructure:** `src/ScreepsDotNet.Engine.Tests/Parity/`
**Parity Harness:** `tools/parity-harness/` (to be created)
