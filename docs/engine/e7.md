...
- ✅ MongoDB 7 for Node.js test harness (automated via Testcontainers.MongoDb)
- ✅ xunit v3 test infrastructure
- ✅ Testcontainers.MongoDb 4.10.0 (automatic MongoDB container lifecycle management)
- ✅ Node.js 10.13.0 to 12.x for running official engine processor (Node 13+ NOT supported)
...
**Deliverables:**
- ✅ `tools/parity-harness/CLAUDE.md` - AI context for subsystem
- ✅ `tools/parity-harness/engine/test-runner/fixture-loader.js` - Load JSON fixtures into MongoDB
- ✅ `tools/parity-harness/engine/test-runner/processor-executor.js` - Execute official Screeps engine with mocked infrastructure
- ✅ `tools/parity-harness/engine/test-runner/output-serializer.js` - Serialize mutations/stats/actionLogs to JSON
- ✅ `tools/parity-harness/engine/test-runner/run-fixture.js` - CLI wrapper (`node run-fixture.js fixture.json --output output.json`)
- ✅ `tools/parity-harness/engine/scripts/clone-repos.sh` - Clone/update official Screeps repos (Linux/Mac)
- ✅ `tools/parity-harness/engine/scripts/clone-repos.ps1` - Clone/update official Screeps repos (Windows PowerShell)
- ✅ `tools/parity-harness/versions.json` - Version pinning configuration
- ✅ `tools/parity-harness/engine/examples/harvest_basic.json` - Example fixture
- ✅ 23 intent types mapped to processor paths

**Impact:**
- ✅ Can execute JSON fixtures using official Node.js Screeps engine
- ✅ Can capture mutations, stats, and action logs from Node.js execution
- ✅ Ready for field-by-field comparison with .NET engine
- ⚠️ Integration with .NET parity tests pending (Phase 5)
...
**Deliverables:**
- ✅ `ParityTestRunner` - Executes RoomProcessor and captures mutations
- ✅ `CapturingMutationWriter` - Captures room object patches, upserts, removals
- ✅ `CapturingStatsSink` - Captures stats changes
- ✅ `CapturingGlobalMutationWriter` - Captures global mutations (GCL, power)
- ✅ 14-step processor pipeline operational (with test doubles)
- ✅ **JSON fixture loader implemented** - `JsonFixtureLoader` and `JsonFixtureSchema` (2026-01-22)
  - ✅ `JsonFixtureSchema.cs` - Record types matching Node.js harness JSON format
  - ✅ `JsonFixtureLoader.cs` - Deserializes JSON to RoomState
  - ✅ 4 example JSON fixtures (harvest_basic, transfer_basic, controller_upgrade, link_transfer)
  - ✅ 7 integration tests covering fixture loading and edge cases
  - ✅ All 532 Engine tests passing

**Impact:**
- ✅ Can execute fixtures programmatically with full type safety (ParityFixtureBuilder)
- ✅ Can execute fixtures from JSON files (JsonFixtureLoader)
- ✅ Can share fixtures with Node.js harness (JSON format compatible)
- ✅ Can capture all mutations for comparison
...
- ✅ `NodeJsHarnessRunner` - Executes Node.js harness and parses JSON output
- ✅ 5 comparison tests validating comparator logic

**Impact:**
- ✅ Can compare mutations field-by-field
- ✅ Can detect divergences automatically
- ✅ Can generate human-readable divergence reports
- ⚠️ Action log and final state comparison deferred (infrastructure ready)
...
**Deliverables:**
- ✅ `ParityFixtureBuilder` - Fluent API for programmatic fixture creation (includes `WithPullIntent`)
- ✅ 52 mechanics tests: Harvest(2), Controller(3), Transfer(3), Link(4), Lab(3), Combat(8), Movement(11), BuildRepair(8), Spawn(7), Nuker(4), PowerSpawn(4), Factory(5)
- ✅ 14 edge case tests: Empty/full stores, overflow, resource limits, TTL, fatigue, cooldowns
- ✅ 7 validation tests: Range checks, resource insufficiency, permissions, invalid targets, cooldowns, body parts
- ✅ All 20 processor steps operational with test doubles
- ✅ Test organization by mechanic family

**Impact:**
- ✅ Comprehensive behavioral validation for E1-E6 features
- ✅ All core mechanics tested (harvest, combat, movement, build/repair, spawn, structures)
- ✅ Edge cases and validation rules covered
- ⚠️ Node.js parity comparison pending (Phase 5 integration)
- ⚠️ Additional mechanics possible (Observer, Terminal, advanced movement)
...
**Deliverables:**
- ✅ `NodeJsOutputSchema.cs` - Strongly-typed schema for Node.js harness JSON output (optional type-safe deserialization)
- ✅ `NodeJsHarnessRunner` - Executes Node.js harness and returns `JsonDocument` for comparison
- ✅ `NodeJsHarnessRunnerTests.cs` - 1 test verifying JsonDocument parsing
- ✅ `EndToEndParityTests.cs` - 3 tests demonstrating .NET Engine execution
- ✅ `ParityFixtureBuilder.WithController()` enhanced - Added `progressTotal` parameter
- ✅ **Comparison infrastructure wired** - `NodeJsHarnessRunner` → `ParityComparator.Compare(ParityTestOutput, JsonDocument)`
- ✅ All 536 Engine tests passing

**What Works:**
- ✅ Can load JSON fixtures using JsonFixtureLoader (7 tests passing)
- ✅ Can execute .NET Engine and capture mutations (3 end-to-end tests)
- ✅ Can invoke Node.js harness via NodeJsHarnessRunner and get JsonDocument output
- ✅ Can compare .NET vs Node.js outputs using existing ParityComparator (5 comparison tests passing)
- ✅ Can create fixtures programmatically with ParityFixtureBuilder
- ✅ **Field-by-field comparison infrastructure complete** - ParityComparator compares mutations, stats, and reports divergences

**MongoDB Integration:**
- ✅ **Parity test category implemented** - `[Trait("Category", "Parity")]` on `EndToEndParityTests`
- ✅ **Automatic prerequisite checking** - `ParityTestPrerequisites` fixture checks Node.js, Docker, repos, npm
- ✅ **nvm support** - Auto-detects nvm, finds suitable Node version, activates via `nvm use`
- ✅ **Testcontainers integration complete** - `MongoDbParityFixture` automatically starts/stops MongoDB 7 container
- ✅ **Full comparison flow wired** - JsonFixture → .NET Engine → ParityTestRunner → ParityComparator → DivergenceReporter
- ✅ **Zero-config setup** - Tests automatically clone repos and install dependencies if missing
- ✅ **Setup documentation** - `mongodb-parity-setup.md` with troubleshooting guide
- ✅ **Test fixtures ready** - 4 JSON fixtures (harvest_basic, transfer_basic, controller_upgrade, link_transfer)
- ✅ **7 parity tests operational** - Harvest (2), Transfer (2), ControllerUpgrade (2), LinkTransfer (1)

**How to Run Parity Tests:**
```bash
# Prerequisites (manual install once):
# - Node.js 10.13.0 to 12.x from https://nodejs.org/dist/latest-v12.x/
#   ⚠️ Node 13+ NOT supported - Screeps is incompatible
#   💡 Recommended: nvm install 12.22.12
...
**What's Pending:**
- 📋 CI/CD automation (GitHub Actions workflow - Phase 6)
- 📋 RoomState → JSON serialization (optional, for programmatic fixture generation)

**Impact:**
- ✅ .NET Engine can run fixtures and produce mutations
- ✅ Node.js harness can run fixtures and produce JsonDocument outputs
- ✅ Can compare outputs field-by-field with ParityComparator
- ✅ Can detect divergences automatically
- ✅ **Can run live parity tests manually** (requires MongoDB + Node.js harness setup)
- ❌ Cannot detect divergences automatically in CI (no workflow yet - Phase 6)
...
- ✅ **nvm detection** (if nvm installed, auto-activates Node 10.13.0-12.x)
- ✅ **Node.js version** 10.13.0 to 12.x ONLY (rejects Node 13+)
- ✅ **Docker running** (fails with helpful error if not running)
- ✅ **Official Screeps repos** cloned in `tools/parity-harness/engine/repos/`
- ✅ **npm dependencies** installed in `tools/parity-harness/engine/node_modules/`
- ✅ **MongoDB 7 container** started by Testcontainers (automatically stopped after tests)

**Expected Output:**
- ✅ All tests pass → 100% parity achieved
- ❌ Divergences detected → Detailed report showing field differences
...
### In Progress 🚧
- [x] **Phase 5:** Integration (JSON fixture loader ✅, NodeJsHarnessRunner wiring ✅, Node.js baseline comparison 📋, CI automation 📋)
  - ✅ JSON fixture loader implemented (JsonFixtureLoader + JsonFixtureSchema)
  - ✅ Node.js harness runner wired (NodeJsHarnessRunner + NodeJsOutputSchema)
  - ✅ End-to-end parity test examples created (3 tests demonstrating .NET execution)
  - 📋 Node.js baseline comparison pending (requires MongoDB setup)
...
**Status:** ✅ Complete - Full harness implemented (2026-01-22)

**What Was Implemented:**
- ✅ `tools/parity-harness/` - Multi-layer parity testing infrastructure
- ✅ `tools/parity-harness/engine/test-runner/fixture-loader.js` - Load JSON fixtures into MongoDB
- ✅ `tools/parity-harness/engine/test-runner/processor-executor.js` - Execute Node.js engine on fixtures with mocked infrastructure
...
- ✅ `tools/parity-harness/engine/scripts/clone-repos.sh` - Clone/update official Screeps repositories (Linux/Mac)
- ✅ `tools/parity-harness/engine/scripts/clone-repos.ps1` - Clone/update official Screeps repositories (Windows PowerShell)
- ✅ `tools/parity-harness/versions.json` - Version pinning configuration
- ✅ `tools/parity-harness/engine/examples/harvest_basic.json` - Example fixture demonstrating format

**Impact:**
- ✅ Can execute JSON fixtures using official Node.js Screeps engine
- ✅ Can capture mutations, stats, and action logs from Node.js execution
- ✅ Can compare .NET output directly to Node.js output field-by-field
- ✅ Can detect divergences from official Screeps engine automatically
- ⚠️ Integration with .NET parity tests not yet wired (awaiting Phase 5: JSON fixture loader in .NET)

**Next Steps (Phase 5: Integration):**
- ✅ Implement JSON fixture loader in .NET (**COMPLETE** - JsonFixtureLoader operational)
...
**Status:** ✅ Complete - 20/20 processor steps operational with test doubles

**What Was Implemented:**
- ✅ `StubCreepDeathProcessor` - Minimal death processing (removes creep without tombstone creation)
- ✅ `StubSpawnIntentParser` - Always returns success (no spawn intent validation)
- ✅ `StubSpawnStateReader` - Returns empty state (no spawning in progress)
- ✅ `StubSpawnEnergyCharger` - Always succeeds (no energy allocation)
- ✅ `StubStructureBlueprintProvider` - Returns null for all structure types
- ✅ `StubStructureSnapshotFactory` - Creates minimal structure snapshots

**Impact:**
- ✅ Can test ALL game mechanics (harvest, transfer, controller, lab, link, movement, combat, build/repair, spawn, tower, lifecycle)
- ✅ Can test validation rules for all intents (range, resources, permissions)
- ✅ Can test passive mechanics (regen, decay, TTL, fatigue, cooldowns)
- ✅ Full processor pipeline operational (all 20 steps execute without errors)
- ⚠️ Test doubles use minimal logic (sufficient for parity tests but not full production behavior)
...
**Impact:**
- ✅ Can create complex fixtures programmatically with full IntelliSense support
- ✅ Fixtures are version-controlled as code (easier to review/diff)
- ❌ Cannot share fixtures with Node.js harness directly (need conversion)
- ❌ Cannot load pre-existing Screeps fixtures from official repos
...
**Impact:**
- ✅ Can validate all game state mutations (store, energy, hits, positions, etc.)
- ✅ Can validate stats recording (energy harvested, control points, etc.)
- ❌ Cannot validate action log entries (x/y coordinates of harvest target, attack direction, etc.)
- ❌ May miss divergences in visualization data (client-side only)
...
**Impact:**
- ✅ Can validate all mutations applied during tick
- ✅ Can detect incorrect mutation logic
- ❌ Cannot detect state divergences caused by missing mutations
- ❌ Cannot validate final state matches Node.js exactly
...
**Status:** ✅ Complete for basic mechanics, advanced structures deferred

**What Was Implemented:**
- ✅ **Combat** (8 tests): Attack (melee), ranged attack, heal (self/others), range validation, out-of-range scenarios
- ✅ **Build/Repair** (8 tests): Construction progress, structure repairs, range validation, energy requirements, WORK part validation, decay mechanics
- ✅ **Movement** (11 tests): Basic move (4 directions + diagonal, fatigue, balanced load) + **Pull mechanics** (chains, range, priority, loops)
- ✅ **Spawn** (7 tests): Renew (TTL increase), recycle (creep removal), range validation, ownership checks, energy/TTL edge cases
...
- **Terminal.send**: Inter-room resource transfers (**Refactored to Intent Pattern** ✅)
  ...
  - **Refactoring Complete (2026-01-22):**
    - ✅ Added `TerminalIntentEnvelope` and `TerminalSendIntent` records to intent contracts
    - ✅ Updated `InterRoomSnapshotBuilder` to convert property→intent at Driver boundary
    - ✅ Refactored `MarketIntentStep.ProcessTerminalSends()` to read from RoomIntents
    - ✅ Added `WithTerminal()` and `WithTerminalSendIntent()` to ParityFixtureBuilder
...
**Status:** ⚠️ **DEFERRED** - Not implemented, documented for future reference
...
- ✅ **Unit Tests Exist:** `MarketIntentStepTests.cs` comprehensively tests terminal sends, market orders, deals
- ✅ **Unit Tests Exist:** `PowerCreepIntentStepTests.cs` covers power creep lifecycle
- ✅ **Deterministic Logic:** Global mechanics are more deterministic than room combat/movement
- ✅ **Well-Documented:** Official Screeps codebase provides clear reference for global mechanics

**Diminishing Returns:**
- **Time Investment:** 8-16 hours to implement vs. limited additional value
- **Complexity:** 5-10× more complex than room parity tests (multi-room fixtures, market infrastructure)
- **ROI:** Better spent on E8 (Advanced Structures), E9 (AI/Vision), or Backend features
- **E7 Scope:** Core goal is validating room mechanics (✅ achieved with 83 tests)

**Terminal.send Specifically:**
- ✅ Intent pattern refactoring complete (architectural consistency achieved)
- ✅ Unit tests validate energy cost calculations, distance logic, capacity checks
- ✅ ParityFixtureBuilder support added (`WithTerminal()`, `WithTerminalSendIntent()`)
- ✅ Can manually test via Backend HTTP/CLI for integration validation
...
```csharp
[Theory]
[InlineData(1, 100, 10)]    // Distance=1, Amount=100 → Cost=10
[InlineData(5, 1000, 500)]  // Distance=5, Amount=1000 → Cost=500
[InlineData(10, 500, 500)]  // Distance=10, Amount=500 → Cost=500
...
**Impact:**
- ✅ Core mechanics comprehensively tested (harvest, transfer, controller, lab, link, combat, movement, build/repair, spawn)
- ✅ Advanced structures tested (Nuker, PowerSpawn, Factory)
- ✅ Edge cases and validation rules tested across all mechanics
- ✅ Pull mechanics fully tested (chains, range checks, priority, loop prevention)
- ⚠️ Tower mechanics not validated (complex targeting, energy allocation - test double limitation)
- ⚠️ Creep creation not validated (body part validation, name uniqueness - stub limitation)
- ❌ **Observer NOT validated** - Feature not implemented in Engine (awaiting E9 or future milestone)
- ⚠️ **Terminal.send NOT validated** - Feature IS implemented, just needs test infrastructure (can add now)
...
public async Task Renew_WithInsufficientEnergy_ProducesNoMutation()
{
    ...
    Assert.Empty(ttlPatches);  // Renew fails ✅
}
...
public async Task TryCharge_SpawnLacksEnergy_PullsFromExtensions()
{
    ...
    Assert.Equal(40, energyLedger[extension.Id]);  // ✅ Pulled from extension
}
```
...
**Impact:**
- ✅ Common edge cases tested (empty/full stores, overflow, insufficient resources)
- ✅ Validation rules tested (range, permissions, cooldowns, body parts)
- ❌ Rare edge cases may have undetected divergences
- ❌ Concurrent action handling not validated
- ❌ Complex multi-step scenarios not validated
...
**Completed:**
- ✅ Test doubles for deferred processor steps (#2) → **All 20 processor steps operational** (Phase 2 complete)
- ✅ Node.js harness (#1) → **Complete** (Phase 1 implemented 2026-01-22)

**Future Enhancements (No Blockers):**
- JSON fixture loading (#3) → Enables Node.js/NET fixture sharing
- Node.js integration (#1 integration) → Wire NodeJsHarnessRunner to execute fixtures
- Additional mechanics tests (#6) → Expands coverage to 40+ fixtures
- Additional edge cases (#8) → Comprehensive edge case validation

**Optional/Low Priority:**
- Action log comparison (#4) → Visualization validation (non-critical)
- Final state comparison (#5) → Infrastructure ready, awaiting integration
- Observer/Terminal tests → No blockers (optional)
- Advanced movement tests → Pull chains, portal mechanics (optional)
- **GlobalParityTestRunner** → Cross-room/global processor testing (documented below, optional)

**Recommended Next Steps:**
1. ✅ ~~Implement test doubles for deferred processor steps~~ **COMPLETE**
2. ✅ ~~Add Nuker/PowerSpawn/Factory tests~~ **COMPLETE** (4 + 4 + 5 = 13 tests)
3. ✅ ~~Add combat/movement/build/repair/spawn parity tests~~ **COMPLETE**
4. ✅ ~~Expand edge case coverage~~ **COMPLETE** (14 edge case tests)
5. ✅ ~~Implement Node.js harness~~ **COMPLETE** (Phase 1 - full harness operational)
6. ✅ ~~Implement JSON fixture loader in .NET~~ **COMPLETE** (JsonFixtureLoader + 7 tests + 4 fixtures)
7. Integrate Node.js harness with .NET tests → Phase 5 (NodeJsHarnessRunner wiring)
8. Add Observer/Terminal tests → No blockers (optional)
9. Add advanced movement tests → Pull chains, portal mechanics (optional)
...
**Status:** 🚧 In Progress (Phase 1-4 ✅, Phase 5 Pending Integration)
...
**Test Infrastructure:** Node.js harness (Phase 1 ✅), ParityComparator, DivergenceReporter, ParityFixtureBuilder, 14-step processor pipeline
