...
**Deliverables:**
- ✅ Range Validator (35 intent types, 30 tests)
- ✅ Resource Validator (20 intent types, 20 tests)
...
- ✅ **`ValidationErrorCode`** - Enum with 45 error codes covering all validation scenarios
...
├── Models/
│   ├── ValidationResult.cs
│   └── ValidationErrorCode.cs (45 error codes)
└── Constants/
...
|-----------|--------|-------|--------------|--------|-----------|
| **E3.2.1 Range** | 📋 | 0/30 | 35 intents | 3-4h | - |
| **E3.2.2 Resource** | 📋 | 0/20 | 20 intents | 2-3h | - |
| **E3.2.3 Permission** | 📋 | 0/20 | 15 rules | 3-4h | - |
| **E3.2.4 State** | 📋 | 0/15 | 10 rules | 2-3h | - |
| **E3.2.5 Schema** | 📋 | 0/15 | 35 intents | 2-3h | - |
| **Total** | **📋 0%** | **0/100** | **All** | **12-17h** | **-** |
...
**Range 1 (Adjacent - Chebyshev distance ≤ 1):**
| Intent Type | Range | Tests | Status | Notes |
|-------------|-------|-------|--------|-------|
| attack | 1 | 0/2 | 📋 | Melee attack |
| harvest | 1 | 0/2 | 📋 | Source/mineral/deposit |
| build | 1 | 0/2 | 📋 | Construction sites |
| repair | 1 | 0/2 | 📋 | Structures |
| dismantle | 1 | 0/2 | 📋 | Structures |
| transfer | 1 | 0/2 | 📋 | Energy/resources to target |
| withdraw | 1 | 0/2 | 📋 | From structures |
| pickup | 1 | 0/2 | 📋 | Resources/tombstones/ruins |
| drop | 1 | 0/1 | 📋 | No target (self) |
| heal | 1 | 0/2 | 📋 | Melee heal |
| claimController | 1 | 0/2 | 📋 | Claim neutral controller |
| generateSafeMode | 1 | 0/2 | 📋 | Activate safe mode |
| pull | 1 | 0/2 | 📋 | Pull another creep |

**Range 3:**
| Intent Type | Range | Tests | Status | Notes |
|-------------|-------|-------|--------|-------|
| rangedAttack | 3 | 0/2 | 📋 | Ranged attack single target |
| rangedHeal | 3 | 0/2 | 📋 | Ranged heal |
| rangedMassAttack | 3 | 0/2 | 📋 | Center at range 3, splash at range 1 |
| upgradeController | 3 | 0/2 | 📋 | Upgrade controller |
| attackController | 3 | 0/2 | 📋 | Downgrade enemy controller |
| reserveController | 3 | 0/2 | 📋 | Reserve neutral controller |
| signController | 3 | 0/2 | 📋 | Sign controller message |

**Special Cases:**
| Intent Type | Range | Tests | Status | Notes |
|-------------|-------|-------|--------|-------|
| towerAttack | 50 | 0/1 | 📋 | Full room range, falloff >5 |
| towerHeal | 50 | 0/1 | 📋 | Full room range, falloff >5 |
| towerRepair | 50 | 0/1 | 📋 | Full room range, falloff >5 |
| spawn | 0 | 0/1 | 📋 | Spawn is actor, no range check |
| renew | 1 | 0/2 | 📋 | Creep must be adjacent to spawn |
| recycle | 1 | 0/2 | 📋 | Creep must be adjacent to spawn |
...
|-------------|----------|-------------------|-------|--------|
| build | energy | `workParts * BUILD_POWER * 0.2 = workParts * 5 * 0.2 = workParts` | 0/2 | 📋 |
| repair | energy | `workParts * REPAIR_POWER * REPAIR_COST = workParts * 100 * 0.01 = workParts` | 0/2 | 📋 |
| upgradeController | energy | `workParts * 1` (boosted: up to 2x with GH compounds) | 0/3 | 📋 |
| spawn | energy | Pre-calculated cost from body parts | 0/2 | 📋 |
| renew | energy | Cost based on creep body | 0/2 | 📋 |

**Mineral/Compound Consumers:**
| Intent Type | Resource | Amount | Tests | Status |
|-------------|----------|--------|-------|--------|
| boostCreep (lab) | mineral | 30 per body part | 0/2 | 📋 |
| boostCreep (lab) | energy | 20 per body part | 0/2 | 📋 |
| unboostCreep (lab) | N/A | Returns 15 mineral (check capacity) | 0/1 | 📋 |
| runReaction (lab) | component1 | 5 per tick (from formula) | 0/2 | 📋 |
| runReaction (lab) | component2 | 5 per tick (from formula) | 0/2 | 📋 |

**Power/Commodity Consumers:**
| Intent Type | Resource | Amount | Tests | Status |
|-------------|----------|--------|-------|--------|
| processPower | power | 1 per tick | 0/2 | 📋 |
| processPower | energy | 50 per tick | 0/2 | 📋 |
| usePower (power creep) | power | By ability/level (PowerConstants) | 0/2 | 📋 |
| produce (factory) | commodities | From recipe components | 0/2 | 📋 |

**Transfer Validators:**
| Intent Type | Check | Tests | Status |
|-------------|-------|-------|--------|
| transfer | Source has resource amount | 0/2 | 📋 |
| transfer | Target capacity not exceeded | 0/2 | 📋 |
...
|------|-----------|------------|-------|--------|
| Upgrade owned controller | upgradeController | `controller.user == creep.user` | 0/2 | 📋 |
| Upgrade reserved controller | upgradeController | `controller.reservation?.user == creep.user` | 0/2 | 📋 |
| Attack enemy controller | attackController | `controller.user != creep.user` | 0/2 | 📋 |
| Reserve neutral controller | reserveController | `controller.user == null && !controller.reservation` | 0/2 | 📋 |
| Claim neutral controller | claimController | `controller.user == null` | 0/2 | 📋 |

**Safe Mode:**
| Rule | Applies To | Validation | Tests | Status |
|------|-----------|------------|-------|--------|
| Block attack in safe mode | attack, rangedAttack, rangedMassAttack | `!(controller.safeMode > gameTime && controller.user != creep.user)` | 0/3 | 📋 |
| Allow owner attacks in safe mode | attack | `controller.user == creep.user` (can attack own creeps) | 0/1 | 📋 |
| Safe mode doesn't block dismantle | dismantle | No safe mode check | 0/1 | 📋 |

**Ramparts:**
| Rule | Applies To | Validation | Tests | Status |
|------|-----------|------------|-------|--------|
| Redirect attack to rampart | attack | If structure behind rampart, attack rampart instead | 0/2 | 📋 |
| Block non-owner access | transfer, withdraw, repair | Check rampart ownership or isPublic flag | 0/2 | 📋 |

**Harvesting Permissions:**
| Rule | Applies To | Validation | Tests | Status |
|------|-----------|------------|-------|--------|
| Owned room harvest | harvest (source) | `controller.user == creep.user` OR no controller | 0/2 | 📋 |
| Reserved room harvest | harvest (source) | `controller.reservation?.user == creep.user` OR no reservation | 0/2 | 📋 |
...
|------|------------|-------|--------|-------|
| Creep not spawning | `!creep.spawning` | 0/2 | 📋 | Applies to all creep intents |
| Creep alive | `creep.hits > 0` | 0/2 | 📋 | Dead creeps can't act |
| Creep exists | `roomObjects.ContainsKey(creepId)` | 0/1 | 📋 | Invalid ID check |
| Power creep spawned | `powerCreep.spawned == true` | 0/2 | 📋 | Power creeps must be spawned |

**Target State:**
| Rule | Validation | Tests | Status | Notes |
|------|------------|-------|--------|-------|
| Target exists | `target != null` | 0/2 | 📋 | Non-existent target ID |
| Target not self | `target.id != actor.id` | 0/1 | 📋 | Can't target self (except drop/suicide) |
| Target has hits | `target.hits > 0` | 0/2 | 📋 | For attackable targets |
| Target has store | `target.store != null` | 0/1 | 📋 | For transfer/withdraw |
| Target type valid | Type matches intent (e.g., source for harvest) | 0/2 | 📋 | Type-specific validation |
...
|-------------|----------------|-------|--------|-------|
| attack | `id` (string) | 0/1 | 📋 | Target object ID |
| harvest | `id` (string) | 0/1 | 📋 | Source/mineral/deposit ID |
| transfer | `id` (string), `resourceType` (string), `amount` (int) | 0/1 | 📋 | Target + resource details |
| withdraw | `id` (string), `resourceType` (string), `amount` (int) | 0/1 | 📋 | Structure + resource details |
| build | `id` (string) | 0/1 | 📋 | Construction site ID |
| upgradeController | `id` (string) | 0/1 | 📋 | Controller ID |
| spawn | `name` (string), `body` (array), `energyStructures` (array) | 0/1 | 📋 | Spawn details |
| move | `x` (int), `y` (int) OR `direction` (int) | 0/1 | 📋 | Position or direction |

**Type Validation:**
| Field Type | Validation | Tests | Status |
|------------|------------|-------|--------|
| String IDs | Non-null, non-empty | 0/2 | 📋 |
| Resource types | Valid resource type constant | 0/2 | 📋 |
| Numeric amounts | >= 0 | 0/2 | 📋 |
| Arrays | Non-null, valid element types | 0/2 | 📋 |

**Sub-total:** 0/15 tests

### Implementation Checklist

- [ ] Create `SchemaValidator.cs`
- [ ] Define required fields per intent type
- [ ] Implement field presence checks
- [ ] Implement type validation (string/int/array)
- [ ] Implement resource type constant validation
- [ ] Write 15 tests
...
- ✅ E3.1 (Core Infrastructure) - `IIntentPipeline`, `IIntentValidator` interfaces
- ⚠️ Validation constants - Need to define ranges/costs from Node.js engine
