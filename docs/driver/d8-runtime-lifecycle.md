...
- `IRuntimeTelemetrySink` provides a single fan-out point for structured telemetry/alerts. The default `LoggingRuntimeTelemetryListener` keeps the original structured logs, while `SchedulerTelemetryListener` feeds into `RuntimeThrottleRegistry` so the runner loop can temporarily delay users whose scripts repeatedly crash or time out. Future metrics/observability providers can plug in by registering another listener without touching the loop code.
