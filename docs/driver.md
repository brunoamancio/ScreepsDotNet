...
| **D8 – Runtime lifecycle** | Provide runtime hooks (make runtime, console, memory, intent persistence). | [src/ScreepsDotNet.Driver/docs/RuntimeLifecycle.md](../src/ScreepsDotNet.Driver/docs/RuntimeLifecycle.md) | RawMemory parity, telemetry events, runtime coordinator with sandbox pooling + bundle caching shipped; telemetry now fans out through `IRuntimeTelemetrySink` so watchdog alerts feed the structured logging/metrics pipeline while cold-sandbox recycle + notifications fire automatically |
