...
| **D8 – Runtime lifecycle** | Provide runtime hooks (make runtime, console, memory, intent persistence). | [src/ScreepsDotNet.Driver/docs/RuntimeLifecycle.md](../src/ScreepsDotNet.Driver/docs/RuntimeLifecycle.md) | RawMemory parity, telemetry events, runtime coordinator with sandbox pooling + bundle caching shipped; watchdog now forces cold sandboxes + notifications when users keep timing out, logging polish pending |
