...
- Native pathfinder binaries now live under `src/native/pathfinder`; `dotnet build` downloads the correct RID package (hash-verified) and `PathfinderService` prefers the native solver when `PathfinderServiceOptions.EnableNative` is true, falling back to the managed A* implementation otherwise.
...
| **D4 – Queues & scheduler** | Provide Redis-backed queue service + worker scheduler for main/runner/processor loops. | [src/ScreepsDotNet.Driver/docs/QueueAndScheduler.md](../src/ScreepsDotNet.Driver/docs/QueueAndScheduler.md) | Queues + worker scheduler complete; telemetry-driven throttling already prevents misbehaving users from starving the runner, but the sink still needs to feed the future observability/logging stack. |
...
| **D6 – Pathfinder** | Ship native solver wrapper + managed bindings with terrain cache loading. | [src/ScreepsDotNet.Driver/docs/Pathfinder.md](../src/ScreepsDotNet.Driver/docs/Pathfinder.md) | Native solver + CI pipeline are done, `PathfinderService` now requires the native binaries whenever `PathfinderServiceOptions.EnableNative` (default) is true, and room callbacks/multi-goal/flee helpers—including `roomCallback` block semantics—flow through the native bridge. `src/native/pathfinder/scripts/run-legacy-regressions.js` replays the regression fixtures against the Node driver (see `reports/legacy-regressions.*`), and `scripts/refresh-baselines.js` copies the canonical output into `ScreepsDotNet.Driver.Tests/Pathfinding/Baselines/legacy-regressions.json`. The January 13, 2026 run matched on path/cost/ops, so the managed solver now exists only as a troubleshooting flag (`EnableNative = false`). Next: grow the fixture set (movement/controller/power cases) and decide when to delete the managed fallback entirely. |
