...
| **D4 – Queues & scheduler** | Provide Redis-backed queue service + worker scheduler for main/runner/processor loops. | [src/ScreepsDotNet.Driver/docs/QueueAndScheduler.md](../src/ScreepsDotNet.Driver/docs/QueueAndScheduler.md) | Queue backlogs now emit stage-tagged telemetry (`idle`, `dequeue`, `drain*`, `throttleDelay`) via the sink, and `WorkerScheduler` surfaces crashes as `Stage=scheduler` events. The remaining gap is wiring the sink into the centralized observability stack (see [src/docs/runtime-telemetry.md](../src/docs/runtime-telemetry.md)). |
