...
| **D8 – Runtime lifecycle** | Provide runtime hooks (make runtime, console, memory, intent persistence). | [src/ScreepsDotNet.Driver/docs/RuntimeLifecycle.md](../src/ScreepsDotNet.Driver/docs/RuntimeLifecycle.md) | RawMemory parity, telemetry events, runtime coordinator with sandbox pooling + bundle caching shipped; telemetry now fans out through `IRuntimeTelemetrySink` and the new scheduler listener uses those signals to throttle users that spam timeouts while watchdog alerts continue to drive cold-sandbox recycle + notifications |
| **D9 – History & notifications** | Save room history/map view diffs and deliver notifications/console output via hooks. | [src/ScreepsDotNet.Driver/docs/HistoryAndNotifications.md](../src/ScreepsDotNet.Driver/docs/HistoryAndNotifications.md) | Room history chunks now persist to Mongo before emitting `roomHistorySaved`; runtime watchdog reuses the notification pipeline. Remainder: blob uploads + throttled history consumers |
