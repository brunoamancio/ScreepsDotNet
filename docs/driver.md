...
| **D10 – Engine contracts** | Expose driver-owned DTOs/providers (room snapshots, mutation batches) so the new .NET engine consumes data via the driver only. | [src/ScreepsDotNet.Driver/docs/EngineContracts.md](../src/ScreepsDotNet.Driver/docs/EngineContracts.md) | **In progress** – Contracts + snapshot builders landed; mutation batching + compatibility shims still underway. |
