...
public interface IMapControlService
{
    ...
    Task OpenRoomAsync(string roomName, CancellationToken cancellationToken = default);

    Task CloseRoomAsync(string roomName, CancellationToken cancellationToken = default);

    Task RemoveRoomAsync(string roomName, bool purgeObjects, CancellationToken cancellationToken = default);

    Task UpdateRoomAssetsAsync(string roomName, bool fullRegeneration, CancellationToken cancellationToken = default);
