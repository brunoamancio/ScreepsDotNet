...
- Native pathfinder binaries are fetched automatically during `dotnet build` for supported runtime identifiers (linux-x64/linux-arm64/win-x64/win-arm64/osx-x64/osx-arm64). Set `NativePathfinderSkipDownload=true` if you need to rely on a local build instead.
