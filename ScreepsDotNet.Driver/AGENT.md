...
- Processor loop now stores per-object `lastIntent*` metadata via `IBulkWriterFactory`, writes intent summaries to map view + event logs, and triggers aggregated intent notifications via `IDriverLoopHooks`. Real combat/economy resolution remains TODO.
