...
- Native pathfinder binaries are fetched automatically during `dotnet build` for supported runtime identifiers (linux-x64/linux-arm64/win-x64/win-arm64/osx-x64/osx-arm64) and the downloaded archive is verified against the `.sha256` manifest from the release. Set `NativePathfinderSkipDownload=true` to keep a local copy or `NativePathfinderSkipHashCheck=true` if you need to bypass verification temporarily.
