...
public sealed class TombstoneDecayStepTests
{
    private readonly TombstoneDecayStep _step;
    private readonly FakeResourceDropHelper _dropHelper;

    public TombstoneDecayStepTests()
    {
        _dropHelper = new FakeResourceDropHelper();
        _step = new TombstoneDecayStep(_dropHelper);
    }

    [Fact]
    public async Task Tombstone_DecayTimePassed_RemovesTombstoneAndDropsResources()
    {
        // Arrange
        var tombstone = CreateTombstone("tombstone1", "W1N1", 10, 10, decayTime: 100, energy: 500, utrium: 100);
        var context = CreateContext([tombstone], gameTime: 100);
        ...
        Assert.Equal("tombstone1", removal);
        ...
        Assert.Contains(_dropHelper.DroppedResources, d => d.ResourceType == ResourceTypes.Utrium && d.Amount == 100);
    }

    [Fact]
    public async Task Tombstone_DecayTimeNotReached_NoMutation()
    {
        // Arrange
        var tombstone = CreateTombstone("tombstone1", "W1N1", 10, 10, decayTime: 200, energy: 500);
        var context = CreateContext([tombstone], gameTime: 100);
        var writer = (FakeMutationWriter)context.MutationWriter;
    ...
    [Fact]
    public async Task Tombstone_DecayTimeExactlyAtThreshold_RemovesTombstone()
    {
        // Arrange - Node.js checks: gameTime >= decayTime - 1
        var tombstone = CreateTombstone("tombstone1", "W1N1", 10, 10, decayTime: 100, energy: 100);
        var context = CreateContext([tombstone], gameTime: 99);
        var writer = (FakeMutationWriter)context.MutationWriter;
    ...
    [Fact]
    public async Task Tombstone_NoDecayTime_RemovesImmediately()
    {
        // Arrange
        var tombstone = CreateTombstone("tombstone1", "W1N1", 10, 10, decayTime: null, energy: 100);
        var context = CreateContext([tombstone], gameTime: 100);
        var writer = (FakeMutationWriter)context.MutationWriter;
    ...
    [Fact]
    public async Task Tombstone_EmptyStore_RemovesWithoutDrops()
    {
        // Arrange
        var tombstone = CreateTombstone("tombstone1", "W1N1", 10, 10, decayTime: 100);
        var context = CreateContext([tombstone], gameTime: 100);
        var writer = (FakeMutationWriter)context.MutationWriter;
    ...
    [Fact]
    public async Task MultipleTombstones_OnlyDecayedOnesRemoved()
    {
        // Arrange
        var tombstone1 = CreateTombstone("tombstone1", "W1N1", 10, 10, decayTime: 100, energy: 100);
        var tombstone2 = CreateTombstone("tombstone2", "W1N1", 20, 20, decayTime: 200, energy: 200);
        var tombstone3 = CreateTombstone("tombstone3", "W1N1", 30, 30, decayTime: 150, energy: 300);
        var context = CreateContext([tombstone1, tombstone2, tombstone3], gameTime: 150);
        var writer = (FakeMutationWriter)context.MutationWriter;
        ...
        Assert.Contains("tombstone1", writer.Removals);
        Assert.Contains("tombstone3", writer.Removals);
        Assert.DoesNotContain("tombstone2", writer.Removals);
    ...
    [Fact]
    public async Task NonTombstoneObjects_Ignored()
    {
        // Arrange
        var creep = CreateCreep("creep1", "W1N1", 10, 10);
        var ruin = CreateRuin("ruin1", "W1N1", 20, 20, decayTime: 100, energy: 500);
        var context = CreateContext([creep, ruin], gameTime: 100);
        var writer = (FakeMutationWriter)context.MutationWriter;
    ...
    [Fact]
    public async Task Tombstone_MultipleResourceTypes_DropsAll()
    {
        // Arrange
        var store = new Dictionary<string, int>
        {
            [ResourceTypes.Energy] = 100,
            [ResourceTypes.Hydrogen] = 50,
            [ResourceTypes.Oxygen] = 75,
            [ResourceTypes.Utrium] = 25
        };
        var tombstone = CreateTombstone("tombstone1", "W1N1", 10, 10, decayTime: 100, store: store);
        var context = CreateContext([tombstone], gameTime: 100);
        ...
        Assert.Contains(_dropHelper.DroppedResources, d => d.ResourceType == ResourceTypes.Hydrogen && d.Amount == 50);
        Assert.Contains(_dropHelper.DroppedResources, d => d.ResourceType == ResourceTypes.Oxygen && d.Amount == 75);
        Assert.Contains(_dropHelper.DroppedResources, d => d.ResourceType == ResourceTypes.Utrium && d.Amount == 25);
    }
    ...
    private static RoomObjectSnapshot CreateTombstone(string id, string roomName, int x, int y, int? decayTime = null, int? energy = null, int? utrium = null, Dictionary<string, int>? store = null)
    {
        ...
        if (utrium.HasValue && !actualStore.ContainsKey(ResourceTypes.Utrium))
            actualStore[ResourceTypes.Utrium] = utrium.Value;
    ...
    private static RoomObjectSnapshot CreateRuin(string id, string roomName, int x, int y, int? decayTime = null, int? energy = null)
    {
        var store = new Dictionary<string, int>();
        if (energy.HasValue)
            store[ResourceTypes.Energy] = energy.Value;
