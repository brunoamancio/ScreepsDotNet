...
3. Integration with existing .NET subsystems (Mongo/Redis storage, runtime service, pathfinder, telemetry).
...
| E2  | Pending  | Data & Storage Model | Define managed representations for room objects, users, power creeps, and bulk ops. Ensure schemas align with `ScreepsDotNet.Storage.MongoRedis`. *Exit:* Domain models + repositories checked in; storage unit tests green. | Screeps schemas, Mongo adapters. |
...
2. Start **E2 – Data & Storage Model** by defining managed representations for room objects/users/power creeps, leveraging `ScreepsDotNet.Storage.MongoRedis`.
