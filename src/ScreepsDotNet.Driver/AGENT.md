...
- Native pathfinder lives in `src/native/pathfinder`; `PathfinderService` now requires those binaries whenever `PathfinderServiceOptions.EnableNative` (default) is true, and the managed A* solver is available only when that flag is disabled for troubleshooting. Regression baselines (multi-room, flee, portal callbacks, and `roomCallback` block semantics) run in `PathfinderNativeIntegrationTests`.
